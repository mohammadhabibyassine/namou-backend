...
npm run prisma:studio   # inspect the database interactively
```

`npm run check` is not defined in this backend; use the explicit lint, format, test, build, and migration commands above. Before opening a change, run at least `npm run lint`, `npm test`, `npm run test:e2e`, `npm run build`, and `npm run format:check`.
